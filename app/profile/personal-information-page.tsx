...
const PersonalInformationPage = (props: Props) => {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      console.log("picked image asset: ", JSON.stringify(assetData, null, 3));
      try {
        const presignData = await FilesService.getPresignUrl({
          fileName:
            assetData.fileName ?? new Date().toISOString() + assetData.mimeType,
          rootFolder: "images",
        });

        console.log("presign url res: ", JSON.stringify(presignData, null, 3));

        setImage(result.assets[0].uri);
      } catch (error) {
        console.log("presign url error:", JSON.stringify(error, null, 3));
      }
    }
  };
